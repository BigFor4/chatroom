...
function ChatInput() {
    ...
    function sendImageMessage(image: string | ReadFileResult) {
        ...
        if (image.length > config.maxImageSize) {
            Message.warning('The file to be sent is too large', 3);
            return;
        }
    ...
    async function handleSendImage() {
        if (!connect) {
            return Message.error('Message sending failed, you are currently offline');
        }
        ...
        <div className={Style.chatInput} {...aero}>
                ...
                overlay={
                    <div className={Style.featureDropdown}>
                        <Menu onClick={handleFeatureMenuClick}>
                            <MenuItem key="huaji">Send emoji</MenuItem>
            ...
            >
                <input
                    ...
                    type="text"
                    placeholder="Let's chat about anything, but please avoid spamming without meaning"
                    maxLength={2048}
                    ref={$input}
